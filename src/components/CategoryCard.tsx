...
export function CategoryCard({ category, label, image, blurb }: Props) {
  return (
    <Link
      to="/menu/$category"
      params={{ category }}
      className="group relative flex-1 overflow-hidden rounded-2xl min-h-[280px] shadow-sm hover:shadow-xl transition-shadow"
    >
      <img
        src={image}
        alt={label}
      ...
      <div className="relative z-10 h-full flex flex-col justify-end p-6 text-white">
        <h3 className="font-display text-3xl font-semibold">{label}</h3>
